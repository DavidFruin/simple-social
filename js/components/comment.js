...
function createCommentElement(comment, options = {}) {
  ...
  div.innerHTML = `
    <div class="comment-header">
      <a href="#/profile/${comment.user_id}" class="comment-user">${displayEmail(comment.user_email || 'User')}</a>
    ...
    <div class="comment-body">${linkifyMentions(escapeHtml(comment.text), comment.mentions)}</div>
  `;
